...
func main() {
	...
	//create metadata
	// 发送元数据是以context上下文的形式发送， 然后会经过protobuffer协议编码， 转化成grpc的header或者trailer信息
	//在客户端发送这些头信息后，收件方会以头信息的形式接收它们
	md := metadata.Pairs(
	...
	//读取服务器端发送来的grpc header信息
	if t, ok := header["timestamp"]; ok {
		log.Printf("timestamp from grpc header\n")
		for i, e := range t {
			fmt.Printf("%d, %s\n", i, e)
		}
	}else {
